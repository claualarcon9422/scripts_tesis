...
#for (bias_value in bias_values) {
  ...
  predicciones_prueba <- nn$feedforward(X_test_b)
  
  # Calcular el R² para las predicciones
  errores <- y_test - predicciones_prueba
...
cat("Coeficiente de Predicción R²:", mejor_r2, "\n")

# Crear vector de errores
errores <- y_train - nn$feedforward(X_train_b)
errores_test <- y_test - nn$feedforward(X_test_b)
